...
import teel.WordBreakSimple.Result;
...
public class ConcatenatedWords {
	...
	public boolean wordBreak_int(String first, int start, Set<String> wordDict)
	{
		boolean res = false;
		if (hm.containsKey(first)) {
			if (hm.get(first) == Result.TRUE) {
				return true;	
			} else {
				return false;
			}
        ...
        for(int i = start; i <= first.length(); i++) {
        		...
        		if (wordDict.contains(firsts)) {
        			if (first.substring(i).isEmpty()) {
        			 ...
        			 res = wordBreak_int(first.substring(i), 1, wordDict);
        			 if (res == true) {
        				 return true;
        			 }
	...
	public static void main(String []args)
	{
		...
		String []words = {"rfkqyuqfjkx","","vnrtysfrzrmzl","gfve","qfpd","lqdqrrcrwdnxeuo",
				"q","klaitgdphcspij","hbsfyfv","adzpbfudkklrw","aozmixr","ife","feclhbvfuk","yeqfqojwtw","sileeztxwjl",
				"ngbqqmbxqcqp","khhqr","dwfcayssyoqc","omwufbdfxu","zhift","kczvhsybloet","crfhpxprbsshsjxd","ilebxwbcto",
				"yaxzfbjbkrxi","imqpzwmshlpj","ta","hbuxhwadlpto","eziwkmg","ovqzgdixrpddzp","c","wnqwqecyjyib","jy",
				"mjfqwltvzk","tpvo","phckcyufdqml","lim","lfz","tgygdt","nhcvpf","fbrpzlk","shwywshtdgmb","bkkxcvg",
				"monmwvytby","nuqhmfj","qtg","cwkuzyamnerp","fmwevhwlezo","ye","hbrcewjxvcezi","tiq","tfsrptug",
				"iznorvonzjfea","gama","apwlmbzit","s","hzkosvn","nberblt","kggdgpljfisylt","mf","h","bljvkypcflsaqe",
				"cijcyrgmqirz","iaxakholawoydvch","e","gttxwpuk","jf","xbrtspfttota","sngqvoijxuv","bztvaal",
				"zxbshnrvbykjql","zz","mlvyoshiktodnsjj","qplci","lzqrxl","qxru","ygjtyzleizme","inx","lwhhjwsl",
		...
		List<String> ls = cw.findAllConcatenatedWordsInADict(words);
